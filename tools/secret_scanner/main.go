...
func main() {
	pathFlag := flag.String("path", "", "File path to scan (uses stdin if empty)")
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Secret Scanner - MCP Tool\n\n")
		fmt.Fprintf(os.Stderr, "Usage:\n")
		fmt.Fprintf(os.Stderr, "  secret_scanner -path <file_path>\n")
		fmt.Fprintf(os.Stderr, "  secret_scanner <file_path>\n")
		fmt.Fprintf(os.Stderr, "  cat <file> | secret_scanner\n\n")
	...
	if filePath != "" {
		...
		if (fi.Mode() & os.ModeCharDevice) != 0 {
			fmt.Fprintln(os.Stderr, "Reading from stdin... (Press Ctrl+D to finish, or use -path flag)")
		}
		reader = os.Stdin
	}
	...
	if len(found) > 0 {
		status = fmt.Sprintf("Found %d potential secret(s) exposed", len(found))
	}
