    ...
    def render_frame(self, status, emoji, text, scroll_top, battery_level, battery_color):
        ...
        if current_image_path not in [None, ""]:
            ...
            if current_image is not None:
                # If we have a cached image, just draw it (checks for path change happen in update_display_data)
...
def update_display_data(status=None, emoji=None, text=None, 
    ...
    # Force reload of image if path changed or provided (to handle overwrites of same file)
    ...
    if text is not None:
        current_text = text
        # If we are setting new text, clear any persisting image unless a new one is provided
...
def send_to_all_clients(message):
    ...
    if "event" in message:
        print(f"[Server v3] Sending event: {message['event']}")
...
def exit_camera_mode():
    global camera_mode, camera_thread, is_capturing
    with camera_lock:
        if not camera_mode:
            print("[Camera v3] exit_camera_mode called but not in camera_mode")
            return
        print("[Camera v3] === Exiting camera mode (cleaning up thread) ===")
...
def on_button_release():
    global camera_mode, camera_mode_button_press_time, camera_mode_button_release_time, is_capturing
    
    # Always forward release event first so Node stops recording
    ...
    if camera_mode:
        if is_capturing:
            print("[Camera v3] Already capturing, ignoring release")
            return
        ...
        if duration < 0.8:
            ...
            if camera_thread is not None:
                ...
                try:
                    ...
                    notification = {"event": "camera_capture", "v3": True}
                    send_to_all_clients(notification)
                    print("[Camera v3] Capture successful")
                except Exception as e:
                    print(f"[Camera v3] Capture failed: {e}")
                finally:
                    is_capturing = False
        else:
...
def handle_client(client_socket, addr, whisplay):
    ...
    print(f"[Socket v2] Client {addr} connected")
    with clients_lock:
    ...
    try:
        ...
        while True:
            ...
            while "\n" in buffer:
                ...
                print(f"[Socket - {addr}] Received line: {line}")
                try:
                    # print(f"[Socket - {addr}] Parsing line: {line}")
                    content = json.loads(line)
                    print(f"[Socket - {addr}] Received JSON: {content}")
                    ...
                    if set_camera_mode is not None:
                        if set_camera_mode:
                            with camera_lock:
                                if not camera_mode and camera_thread is None:
                                    try:
                                        print(f"[Camera v3] Entering camera mode with path: {camera_capture_image_path}")
                                        camera_mode = True
                                        camera_thread = CameraThread(whisplay, camera_capture_image_path)
                                        camera_thread.start()
                                        print("[Camera v3] Camera thread started successfully")
                                        
                                        # Start monitoring thread to detect error exits
                                        def monitor_camera_errors():
                                            time.sleep(1)  # Give camera time to start
                                            while camera_thread and camera_thread.is_alive():
                                                time.sleep(0.5)
                                            
                                            # Camera thread has stopped, check if it was due to errors
                                            if camera_thread and camera_thread.error_exit:
                                                print("[Camera Monitor] Camera exited due to errors, showing message")
                                                update_display_data(
                                                    status="Camera Error",
                                                    emoji="❌",
                                                    text="Remote camera server disconnected",
                                                    scroll_speed=0
                                                )
                                                time.sleep(3)
                                                exit_camera_mode()
                                        ...
                                        threading.Thread(target=monitor_camera_errors, daemon=True).start()
                                    except Exception as cam_error:
                                        error_message = str(cam_error)
                                        print(f"[Camera] Failed to start camera: {error_message}")
                                        import traceback
                                        traceback.print_exc()
                                        camera_mode = False
                                        camera_thread = None
                                        ...
                                        update_display_data(
                                            status="Camera Error",
                                            emoji="❌",
                                            text=f"Camera unavailable: {error_message}",
                                            scroll_speed=0
                                        )
                                        
                                        # Send error notification to client
                                        try:
                                            error_response = json.dumps({"event": "camera_error", "message": error_message}).encode("utf-8") + b"\n"
                                    ...
                                    print("[Camera] Already in camera mode, ignoring start request")
                        else:
                            exit_camera_mode()
        ...
        print(f"[Socket v2] Client {addr} disconnected")
        with clients_lock:
            if addr in clients:
                del clients[addr]
        client_socket.close()
...
if __name__ == "__main__":
    ...
    try:
        # Keep the main thread alive (though start_socket_server is already blocking)
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
